import type { Card, ExecuteCommand, ExecuteState, MedalTone } from '@/game/types'

/**
 * 模拟层：纯函数，是游戏规则的唯一权威。
 * `pick` 只允许抓取"已整备区"右侧第一件——这正是插入排序的循环变量 j；
 * `shift` 必须先 `compare` 且结论为 greater；`drop` 在空槽到达最左时可免比对（短路边界）。
 * 渲染层与存档层对具体规则无感知。
 */
...
/* —— 指挥关：程序执行器 ——
 * 程序 = 指令模块序列 + 指令指针。每步执行一条指令：动作指令走 executeCommand
...
/** 指挥关奖章按指令数（指令槽稀缺）：金 ≤par、银 ≤par+2、铜=完成。 */
...
/* —— 归并关：双队首选择 ——
 * 两股已排序的恒星流，每次只能取队首；取了较大的一侧立即被拒（这就是汇合不变量的身体感受）。 */
...
/** 归并关奖章按取星次数：金 = 零失误（n 次）、银 ≤n+2、铜=完成。 */
